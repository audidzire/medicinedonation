...
class DatatablesView(MultipleObjectMixin, View):
    ...
    def get_db_fields(self):
        if not self._db_fields:
            self._db_fields = []
            fields = list(self.fields.values()) if isinstance(self.fields, dict) else self.fields
            for field in fields:
    ...
    def get_orders(self):
        ...
        dt_orders = [(self.dt_data['iSortCol_%s' % i], self.dt_data['sSortDir_%s' % i]) for i in range(iSortingCols)]
